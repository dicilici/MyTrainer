...
- `DatabasePlugin/daemon/setup.sh` (+ `databaseplugin.service`)
- `TrainPlugin/daemon/setup.sh` (+ `trainplugin.service`)

Running `setup.sh` **automatically performs the installation**: it copies the
service unit into `/etc/systemd/system/`, generates the environment-variable
template, and runs `systemctl enable` + start.

```bash
cd DatabasePlugin/daemon && sudo bash setup.sh   # Data Node
cd TrainPlugin/daemon && sudo bash setup.sh       # Training Node
```

Installation notes:

- **Binary/package location**: the Data Node is `/opt/databaseplugin`, the
  Training Node is `/opt/trainplugin` (`setup.sh` verifies that the file exists).
...
- **Overwrite note**: `setup.sh` copies the service unit into
  `/etc/systemd/system/<service-name>.service` with `cp`, so an existing file of
  the same name at the target path **will be overwritten**; the environment
  template (`/etc/default/<service-name>`) is skipped (not overwritten) if it
  already exists.
