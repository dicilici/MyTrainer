...
5. The training node buffers data (current task in memory, queued tasks written to disk under `DATA_PATH`), starts training upon receiving `Finish`, and saves the model.
...
limits how long a task may wait for its preprocessed data. The format combines
`d`/`h`/`m`/`s` units freely, e.g. `90m` or `1h30m`. The timer starts when the task
leaves the queue and enters the prepare phase (waiting for data), and is cancelled
once the data is ready — so it covers only the data-waiting phase, excluding both
the time spent queued and the actual training time. When it expires:
...
> Before assigning `train_backend_url` / `train_data_url`, run `checknode` to
> view the machine status of the target training/data nodes (see §6).
...
| `ModelSave`       | string | Model folder path (one `.pt` + one `.json`, see below) |
...
### Model folder on the training node

`ModelSave` is an **absolute path to a folder** (not a file) on the training node.
That folder must contain:

- exactly **one `*.pt`** file — the model weights (a `state_dict`, optionally
  wrapped as `{"state_dict": ...}`);
- exactly **one `*.json`** file — the model structure definition.

The JSON structure definition has the form:
...
- `name` selects one of the built-in models: `ImageCNN` (image input `[B, C, H, W]`),
  `TextClassifier` (text input `[B, L]`), or `MixClassifier` (image + text input);
- `args` are the keyword arguments passed to the model constructor;
- `input_type` / `num_classes` are validated against the dataset's `Input` /
  `CategoriesNumber` and must match.

To place a model, prepare such a folder on the training node and point `ModelSave`
at it. When training finishes, the training node overwrites the existing `*.pt`
file with the trained weights.
...
- **Task time limit**: the `TimeOut` timer starts when a task enters the prepare
  phase and covers only the data-waiting phase (queued time and training time are
  excluded); on expiry the training node cancels the task (reported as
  `<task-id>:timeout`) and deletes its spool file, while the data node stops
  processing that task's data and cleans it up once it observes
  `no matching training task`.
- **Data error / eviction**: if the data node reports an error for a task, the
  training node marks it `failed` and aborts it — a queued task is evicted (its
